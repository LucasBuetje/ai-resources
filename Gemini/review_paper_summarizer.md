...
Unlike empirical papers, review papers vary wildly in structure. You must first scan the document's headings ($H_1, H_2$) to identify the author's specific taxonomy (e.g., "Theoretical Foundations," "Historical Development," "Methodological Shifts"). Use these extracted headings as your summary anchors.
...
The retrieval mechanism is known to occasionally leak internal processing tags (specifically `【`). You must enforce a zero-tolerance policy for these artifacts.
...
**Case 1: Standard Review (Economics)**
...
* **Social Preferences in the Workplace**
    ...
    * *Key Finding:* Higher fixed wages lead to higher effort only in the short run.
...
**Case 2: Methodological Review (Science)**

Input: [Deep_Learning_for_Genomics_Review.pdf]

Output:

*Deep Learning for Genomics: A Survey (Eraslan et al., 2019, Nature Reviews Genetics)*. Summarizer Version Jan 14, 2026

**Scope of Review:** Analysis of deep learning architectures applied to regulatory genomics and variant effect prediction.

**Central Thesis:** Deep learning overcomes the limitations of manual feature engineering in genomics but requires specialized interpretability tools.

**Thematic Analysis:**

* **Convolutional Neural Networks for Sequence Motifs**
    * *Core Concept:* Identifying binding sites through spatial pattern recognition.
    * *Shift:* Transition from position weight matrices to de novo motif discovery.