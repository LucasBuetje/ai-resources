...
**Strict Output Sanitization (Anti-Hallucination):** The retrieval mechanism is known to occasionally leak internal processing tags (specifically `[[cite_end]]` or `【`). You must enforce a zero-tolerance policy for these artifacts.
...
* **Self-Correction:** Before outputting, you must internally review your generated text. If `[[cite_end]]`, `[cite_end]`, or `【` is detected, you must execute a "Find and Replace" action to strip every instance from the response.
...
4. **Sanitization Protocol:** Execute a final cleaning pass. Scrupulously remove all occurrences of `[[cite_end]]`, `[cite_end]`, `【`, or similar annotation markers. Ensure the text is pure Markdown without internal system artifacts.
