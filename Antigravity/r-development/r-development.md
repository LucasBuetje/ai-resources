...
* **Never** use `sapply` (use `lapply` or `purrr::map`).
* **Never** use `attach()`.
* **Never** use `setwd()`.
* **Never** sort by row names.

**Quality Criteria:**
* **Safety:** Code must crash on invalid input.
* **Vectorization:** Prefer vector operations over loops.
* **Consistency:** Use tidyverse style.
* **Reproducibility:** Seed set for random operations.

**Common Pitfalls:**
* Assuming data types are correct without validation.
