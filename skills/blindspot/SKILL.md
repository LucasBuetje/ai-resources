...
**Before starting:** if a file path was provided as argument, read the file (`Read`; `Bash(ls*)` for a directory of outputs) so the audit works from the actual artifact. If no path was provided, ask the user to paste or describe the output — never audit from memory of what the output "should" look like.
...
After working through all four quadrants, produce a Blindspot Report in this format:
...
**Where to write it.** If the project has an `obsidian/` symlink, write the report to
`obsidian/claude-output/YYYY-MM-DD_blindspot_<short-slug>.md`, where `<short-slug>`
describes the audited output (e.g. `event-study-main`). Always also print the Ruling and
the FLAGGED findings to chat so they are visible immediately. If there is no `obsidian/`
symlink, print the full report to chat only — do not create a stray file elsewhere.
